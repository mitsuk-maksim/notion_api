...
class NotionClient(object):
    ...
    def download_block(
            self, block_id, export_type, output_file_name, event_name="exportBlock", 
            recursive=False, time_zone="Europe/Moscow", locale="en", pdf_format="Letter"):
        """
        ...
        tmp = f"{output_file_name}.pdf" if export_type == 'pdf' else 'tmp.zip'
        self._download_url(url, tmp)
        
        if export_type != 'pdf':
            self._unzip_file(tmp)
